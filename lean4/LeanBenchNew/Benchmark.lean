...
General-purpose benchmarking helpers with a small API surface. This is intended
as a shared core that can be reused across projects.
-/
...
/-- Benchmark logger interface. -/
structure Logger where
  logFields : List (String × LogValue) → IO Unit := fun _ => pure ()
  logResult : Result → Config → IO Unit := fun _ _ => pure ()
...
def run [Monad m] [MonadLiftT IO m] (name : String) (config : Config := {})
  ...
  let result := {
    name := name
    avgTimeNs := if config.timedIterations == 0 then 0 else total / config.timedIterations
    minTimeNs := sorted.headD 0
    maxTimeNs := sorted.getLastD 0
    iterations := config.timedIterations
  }
  return result

/-- Run a benchmark and pass the result to a logger. -/
def runWithLogger [Monad m] [MonadLiftT IO m]
    (logger : Logger := {}) (name : String) (config : Config := {}) (f : Unit → m α) : m Result := do
  let result ← run name config f
  liftM (m := IO) <| logger.logResult result config
  return result
