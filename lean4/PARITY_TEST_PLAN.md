...
| scatter dim-mismatch semantics | `indexing.runtime.scatter_dim_mismatch` | ported | Verifies `index.shape[dim] < self.shape[dim]` lane for scatter/scatter_reduce parity, include-self reducers, scalar `reduce='add'/'multiply'` modeling, and signed-dim (`dim=-1`) wrappers. |
