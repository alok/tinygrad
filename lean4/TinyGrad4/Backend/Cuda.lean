import TinyGrad4.Backend.Device

/-!
# CUDA Backend

Type-safe CUDA GPU backend via FFI to CUDA Driver API.

## Architecture

```
Cuda.lean (Lean typeclasses)
    ↓
tg4_cuda.cu (C/CUDA FFI)
    ↓
CUDA Driver API + NVRTC
    ↓
NVIDIA GPU
```

All buffer and program handles are opaque to Lean,
ensuring type safety and preventing misuse.
-/

namespace TinyGrad4.Backend.Cuda

open TinyGrad4.Backend

/-! ## Opaque FFI Types -/

/-- Opaque CUDA buffer handle -/
opaque CUDABufferImpl : NonemptyType
def CUDABuffer : Type := CUDABufferImpl.type
instance : Nonempty CUDABuffer := CUDABufferImpl.property

/-- Opaque CUDA program handle -/
opaque CUDAProgramImpl : NonemptyType
def CUDAProgram : Type := CUDAProgramImpl.type
instance : Nonempty CUDAProgram := CUDAProgramImpl.property

/-! ## FFI Declarations -/

/-- Allocate a CUDA buffer with n float elements -/
@[extern "tg4_cuda_alloc"]
opaque cudaAlloc : @& Nat → IO CUDABuffer

/-- Free a CUDA buffer -/
@[extern "tg4_cuda_free"]
opaque cudaFree : @& CUDABuffer → IO Unit

/-- Copy data from host FloatArray to CUDA buffer -/
@[extern "tg4_cuda_copy_in"]
opaque cudaCopyIn : @& CUDABuffer → @& FloatArray → IO Unit

/-- Copy data from CUDA buffer to host FloatArray -/
@[extern "tg4_cuda_copy_out"]
opaque cudaCopyOut : @& CUDABuffer → IO FloatArray

/-- Get buffer size in elements -/
@[extern "tg4_cuda_size"]
opaque cudaSize : @& CUDABuffer → IO Nat

/-- Compile CUDA kernel source to executable program via NVRTC -/
@[extern "tg4_cuda_compile"]
opaque cudaCompile : @& String → @& String → IO CUDAProgram

/-- Launch a CUDA kernel -/
@[extern "tg4_cuda_launch"]
opaque cudaLaunch : @& CUDAProgram → @& (Array CUDABuffer) →
    @& Nat → @& Nat → @& Nat → @& Nat → @& Nat → @& Nat → IO Unit

/-- Wait for all GPU work to complete -/
@[extern "tg4_cuda_sync"]
opaque cudaSync : IO Unit

/-- Get CUDA device name -/
@[extern "tg4_cuda_device_name"]
opaque cudaDeviceName : IO String

/-! ## CUDA Kernel Renderer -/

/-- Render a simple ewise CUDA kernel (simplified for now) -/
def renderCUDAKernel (name : String) (_nodes : List Unit) (_outId : Nat) : String :=
  -- Simple add kernel template - will be expanded for real UOps later
  "extern \"C\" __global__ void " ++ name ++ "(\n" ++
  "    const float* __restrict__ a,\n" ++
  "    const float* __restrict__ b,\n" ++
  "    float* __restrict__ out\n" ++
  ") {\n" ++
  "    int gid = blockIdx.x * blockDim.x + threadIdx.x;\n" ++
  "    out[gid] = a[gid] + b[gid];\n" ++
  "}\n"

/-! ## Typeclass Instances -/

instance : Allocator CUDABuffer where
  alloc := cudaAlloc
  free := cudaFree
  copyIn := cudaCopyIn
  copyOut := cudaCopyOut
  size _buf := 0  -- TODO: track size in wrapper struct

instance : Compiler CUDAProgram where
  compile := cudaCompile

instance : Runtime CUDAProgram CUDABuffer where
  launch prog bufs params := do
    let (gx, gy, gz) := params.globalSize
    let (lx, ly, lz) := params.localSize
    cudaLaunch prog bufs gx gy gz lx ly lz
  sync := cudaSync

/-- CUDA renderer for ewise kernels -/
def cudaRenderer : Renderer where
  name := "CUDA"
  renderEwise name nodes outId := some (renderCUDAKernel name (nodes.map fun _ => ()) outId.id)
  renderReduce name _op axes _outer _inner :=
    s!"// TODO: CUDA reduce kernel: {name}, axes={repr axes}"
  renderMatmul m k n :=
    s!"// TODO: CUDA matmul [{m}x{k}] @ [{k}x{n}]"

/-- Complete CUDA device configuration -/
def cudaDevice : Device CUDAProgram CUDABuffer := {
  name := "CUDA"
  allocator := inferInstance
  compiler := inferInstance
  runtime := inferInstance
  renderer := cudaRenderer
}

/-! ## High-Level API -/

/-- Check if CUDA is available -/
def isAvailable : IO Bool := do
  try
    let _ ← cudaDeviceName
    return true
  catch _ =>
    return false

/-- Get GPU device info -/
def deviceInfo : IO String := do
  let name ← cudaDeviceName
  return s!"CUDA Device: {name}"

/-- Simple test: allocate, write, read back -/
def testRoundtrip (data : FloatArray) : IO FloatArray := do
  let buf ← cudaAlloc data.size
  cudaCopyIn buf data
  let result ← cudaCopyOut buf
  cudaFree buf
  return result

/-- Compile and run a simple kernel -/
def runKernel (source : String) (kernelName : String)
    (inputs : Array FloatArray) (outputSize : Nat) : IO FloatArray := do
  -- Allocate buffers
  let mut bufs : Array CUDABuffer := #[]
  for input in inputs do
    let buf ← cudaAlloc input.size
    cudaCopyIn buf input
    bufs := bufs.push buf

  -- Output buffer
  let outBuf ← cudaAlloc outputSize
  bufs := bufs.push outBuf

  -- Compile and launch
  let prog ← cudaCompile kernelName source
  cudaLaunch prog bufs outputSize 1 1 256 1 1
  cudaSync

  -- Copy result
  let result ← cudaCopyOut outBuf

  -- Free
  for buf in bufs do
    cudaFree buf

  return result

end TinyGrad4.Backend.Cuda
