...
/-- Greedy fusion selection: prefer safe fused matmul plans. -/
  ...
  for u in nodes.reverse do
    ...
    if u.op == .ADD || u.op == .MAX then
      ...
      | some p =>
        if setIntersects p.cover covered then
          continue
        implMap := implMap.insert p.root (.fusedMatmul p)
    ...
    if u.op == .FDIV then
      ...
      | some p =>
        if setIntersects p.cover covered then
          continue
        implMap := implMap.insert p.root (.fusedSoftmax p)
        covered := setUnion covered p.cover
        continue
      | none => pure ()
    if u.op == .REDUCE_AXIS then
      ...
      | some p =>
        if setIntersects p.cover covered then
          continue
        implMap := implMap.insert p.root (.fusedReduce p)
        covered := setUnion covered p.cover
        continue
      | none => pure ()
    match FusedEwise.compile u keep refCnt with
    | some p =>
      if setIntersects p.cover covered then
        continue
      implMap := implMap.insert p.root (.fusedEwise p)
      covered := setUnion covered p.cover
    | none => pure ()
