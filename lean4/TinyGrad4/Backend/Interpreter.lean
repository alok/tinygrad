...
def map (f : Float → Float) (arr : FloatArray) : FloatArray := Id.run do
  let mut out := FloatArray.emptyWithCapacity arr.size
  for i in [:arr.size] do
    out := out.push (f arr[i]!)
  return out

def zipWith (f : Float → Float → Float) (a b : FloatArray) : FloatArray := Id.run do
  let n := min a.size b.size
  let mut out := FloatArray.emptyWithCapacity n
  for i in [:n] do
    out := out.push (f a[i]! b[i]!)
  return out

def all (p : Float → Bool) (arr : FloatArray) : Bool := Id.run do
  let mut ok := true
  for i in [:arr.size] do
    ok := ok && p arr[i]!
  return ok
...
def ofF32 (a : FloatArray) : RawBuffer :=
  { dtype := .float32, data := Native.packF32FromF64 a }

def decode (b : RawBuffer) : FloatArray :=
  match b.dtype with
  | .float32 =>
    Native.unpackF64FromF32 b.data
  | .bool =>
    Id.run do
      let mut out := FloatArray.emptyWithCapacity b.data.size
      for i in [:b.data.size] do
        let v : Float := if b.data.get! i == 0 then 0.0 else 1.0
        out := out.push v
      return out
  | _ =>
    ⟨#[]⟩
...
def zeros (n : Nat) : FlatArray := ⟨Array.replicate n 0.0⟩
def full (n : Nat) (v : Float) : FlatArray := ⟨Array.replicate n v⟩
...
private def cmpF32 (x y : RawBuffer) (xShape yShape outShape : Shape) (pred : Float → Float → Bool) : ByteArray := Id.run do
  ...
  let xs := Native.unpackF64FromF32 xData
  let ys := Native.unpackF64FromF32 yData
...
private def evalNode (u : UOp) (env : Env) (cache : HashMap UOpId RawBuffer) : RawBuffer :=
  ...
  | .CAST =>
    Id.run do
      ...
      | .float32 =>
        ...
        return { dtype := .float32, data := Native.packF32FromF64 out }
      | .bool =>
  ...
  def evalCompiled (c : Compiled) (env : Env) : HashMap UOpId FlatArray :=
    let raw := evalCompiledRaw c env
    raw.fold (init := (∅ : HashMap UOpId FlatArray)) fun acc uid buf =>
      acc.insert uid buf.decode

  def evalManyRaw (roots : List UOp) (env : Env) : HashMap UOpId RawBuffer := Id.run do
    evalCompiledRaw (compileMany roots) env

  def evalManyRawCached (roots : List UOp) (env : Env) : IO (HashMap UOpId RawBuffer) := do
    let compiled ← compileManyCached roots
    pure (evalCompiledRaw compiled env)

  def evalManyRawTimed (roots : List UOp) (env : Env) : IO (HashMap UOpId RawBuffer × Nat × Nat) := do
  ...
  def evalManyRawCachedTimed (roots : List UOp) (env : Env) : IO (HashMap UOpId RawBuffer × Nat × Nat) := do
    let (compiled, compileNs) ← compileManyCachedTimed roots
    let (cache, evalNs) ← evalCompiledRawTimedTotal compiled env
    pure (cache, compileNs, evalNs)

  def evalRawCached (u : UOp) (env : Env) : IO RawBuffer := do
    let cache ← evalManyRawCached [u] env
    pure (cache.getD u.uid (RawBuffer.zeros u.dtype (listProd u.shape)))

  def evalManyCached (roots : List UOp) (env : Env) : IO (HashMap UOpId FlatArray) := do
    let raw ← evalManyRawCached roots env
    pure <| raw.fold (init := (∅ : HashMap UOpId FlatArray)) fun acc uid buf =>
      acc.insert uid buf.decode

  def evalManyTimed (roots : List UOp) (env : Env) : IO (HashMap UOpId FlatArray × Nat × Nat) := do
    let (raw, compileNs, evalNs) ← evalManyRawTimed roots env
    let decoded := raw.fold (init := (∅ : HashMap UOpId FlatArray)) fun acc uid buf =>
      acc.insert uid buf.decode
    pure (decoded, compileNs, evalNs)

  def evalManyCachedTimed (roots : List UOp) (env : Env) : IO (HashMap UOpId FlatArray × Nat × Nat) := do
    let (raw, compileNs, evalNs) ← evalManyRawCachedTimed roots env
    let decoded := raw.fold (init := (∅ : HashMap UOpId FlatArray)) fun acc uid buf =>
      acc.insert uid buf.decode
    pure (decoded, compileNs, evalNs)

  def evalCached (u : UOp) (env : Env) : IO FlatArray := do
    let raw ← evalRawCached u env
    pure raw.decode

def evalRaw (u : UOp) (env : Env) : RawBuffer :=
  let cache := evalManyRaw [u] env
  cache.getD u.uid (RawBuffer.zeros u.dtype (listProd u.shape))

def evalMany (roots : List UOp) (env : Env) : HashMap UOpId FlatArray :=
  let raw := evalManyRaw roots env
  raw.fold (init := (∅ : HashMap UOpId FlatArray)) fun acc uid buf =>
    acc.insert uid buf.decode

def eval (u : UOp) (env : Env) : FlatArray :=
  (evalRaw u env).decode

def evalTensor {s : List Nat} {d : DType} (t : StaticTensor s d) (env : Env := ∅) : FlatArray :=
  eval t.uop env

def evalTensorRaw {s : List Nat} {d : DType} (t : StaticTensor s d) (env : Env := ∅) : RawBuffer :=
  evalRaw t.uop env

def evalTensorCached {s : List Nat} {d : DType} (t : StaticTensor s d) (env : Env := ∅) : IO FlatArray :=
  evalCached t.uop env

def evalTensorRawCached {s : List Nat} {d : DType} (t : StaticTensor s d) (env : Env := ∅) : IO RawBuffer :=
  evalRawCached t.uop env
