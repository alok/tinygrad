...
inductive Ops where
  ...
  | BINARY          -- Compiled binary (bytes arg)
...
/-- Operation groups for optimization (matches Python GroupOp) -/
structure GroupOp where
  ...
  elementwise : List Ops := [.EXP2, .LOG2, .SIN, .COS, .TAN, .SQRT, .RECIPROCAL, .NEG, .TRUNC,
                             .ADD, .MUL, .SUB, .FDIV, .IDIV, .MAX, .MOD, .POW,
                             .SHL, .SHR, .XOR, .OR, .AND, .CMPLT, .CMPNE, .CMPEQ, .THREEFRY,
                             .WHERE, .MULACC, .CAST, .BITCAST]

  -- Defines
  defines : List Ops := [.DEFINE_GLOBAL, .DEFINE_LOCAL, .DEFINE_REG]

  -- Irreducible (ops that cannot be simplified)
  irreducible : List Ops := [.CONST, .DEFINE_VAR, .SPECIAL, .RANGE]

  -- Buffer ops
  buffer : List Ops := [.LOAD, .STORE, .CONST, .DEFINE_VAR]

  -- Algebraic properties
  commutative : List Ops := [.ADD, .MUL, .MAX, .CMPNE, .CMPEQ, .XOR, .AND, .OR]
  associative : List Ops := [.ADD, .MUL, .AND, .OR, .MAX]
  idempotent : List Ops := [.OR, .AND, .MAX]

  -- Comparison ops (change dtype to bool)
  comparison : List Ops := [.CMPLT, .CMPNE, .CMPEQ]

  -- Ops that don't preserve f(0) = 0 (unsafe for padding)
  unsafePad : List Ops := [.RECIPROCAL, .LOG2, .EXP2, .IDIV, .POW]
