...
/-- Smoke test: run Triton matmul + bias + bias2 kernel and compare a few samples to expected. -/
def main : IO UInt32 := do
  let available ← TinyGrad4.Backend.Cuda.isAvailable
  if !available then
    IO.println "Linear Triton bias2 smoke: skipped (CUDA not available)"
    return 0
  ...
  let cfg? ←
    ...
    | none => pure none
    | some preset => TinyGrad4.Backend.CudaTritonMatmul.ensureConfigBias preset m n k (bias2 := true)

  match cfg? with
  | none =>
    IO.println "Linear Triton bias2 smoke: FAIL (no Triton bias2 config available)"
    return 1
  | some cfg =>
    let aBytes := fillF32 (m * k) 1.0
    let bBytes := fillF32 (k * n) 1.0
    let biasBytes := fillF32 n 0.5
    let bias2Bytes := fillF32 n 0.25
    ...
    let bias2Buf : RawBuffer := { dtype := .float32, data := bias2Bytes }

    let tritonOut ← TinyGrad4.Backend.CudaTritonMatmul.matmulF32ViaF16Bias2 cfg aBuf bBuf biasBuf bias2Buf m k n
    let expected := roundF32ToF16 ((Float64.ofNat k + 0.5 + 0.25).toFloat32)
    ...
    if ok then
      IO.println "Linear Triton bias2 smoke: ok"
      return 0
    else
      IO.println "Linear Triton bias2 smoke: FAIL"
      return 1
