...
/-- Smoke test: run batched Triton matmul + bias kernel and compare samples. -/
def main : IO UInt32 := do
  let available ← TinyGrad4.Backend.Cuda.isAvailable
  if !available then
    IO.println "Linear Triton bias batched smoke: skipped (CUDA not available)"
    return 0
  ...
  match cfg? with
  | none =>
    IO.println "Linear Triton bias batched smoke: FAIL (no Triton bias config available)"
    return 1
  | some cfg =>
    let aBytes := fillF32 (batch * m * k) 1.0
    let bBytes := fillF32 (batch * k * n) 1.0
    ...
    let tritonOut ← TinyGrad4.Backend.CudaTritonMatmul.matmulBatchedF32ViaF16Bias cfg aBuf bBuf biasBuf m k n aStarts bStarts biasStarts
    let expected := roundF32ToF16 ((Float64.ofNat k + 0.5).toFloat32)
    let vals := TinyGrad4.RawBuffer.unpackF32Bytes tritonOut.data
    let samples := sampleIndices (batch * m * n)
    let mut ok := true
    ...
    if ok then
      IO.println "Linear Triton bias batched smoke: ok"
      return 0
    else
      IO.println "Linear Triton bias batched smoke: FAIL"
      return 1
