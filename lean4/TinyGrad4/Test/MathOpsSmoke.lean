...
private def assertAllBetween (vals : FloatArray) (lo hi : Float) (label : String) : IO Unit := do
  for i in [:vals.size] do
    let v := vals[i]!
    if v < lo || v >= hi then
      throw (IO.userError s!"{label}: idx {i} {v} not in [{lo}, {hi})")

private def assertEqF32 (got : FloatArray) (expected : Array Float) (label : String) : IO Unit := do
...
def testCompareEval : IO Unit := do
  let (eqT, neT, gtT) := runTensorM do
    ...
    let gtT ← cmpgt c a
    pure (eqT, neT, gtT)
  let eqRaw := evalTensorRaw eqT
  let neRaw := evalTensorRaw neT
  let gtRaw := evalTensorRaw gtT
...
def testBoolOpsEval : IO Unit := do
  let (andT, orT, xorT) := runTensorM do
    ...
    pure (andT, orT, xorT)
  let andRaw := evalTensorRaw andT
  let orRaw := evalTensorRaw orT
  let xorRaw := evalTensorRaw xorT
...
def testMovementInt16 : IO Unit := do
  ...
  let permRaw := evalTensorRaw permT env
  ...
  let shrinkRaw := evalTensorRaw shrinkT env
  ...
  let padRaw := evalTensorRaw padT env
...
def testMaxMinArg : IO Unit := do
  ...
  let argmaxRaw := evalTensorRaw argmaxT
  ...
  let argminRaw := evalTensorRaw argminT
