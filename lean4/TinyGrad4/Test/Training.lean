...
def initWeights : TrainState :=
  ...
  { w1Data := RawBuffer.ofF32 ⟨w1⟩, w2Data := RawBuffer.ofF32 ⟨w2⟩ }
...
def trainStep (p : Program) (state : TrainState) : IO (Float × TrainState) := do
  ...
  let xData : FlatArray := ⟨xData'⟩
  let yData : FlatArray := ⟨yData'⟩

  -- Set up environment with current values
  let env : Env := (∅ : Env)
    |>.insert p.xId (RawBuffer.ofF32 xData)
    |>.insert p.yId (RawBuffer.ofF32 yData)
...
def train (numSteps : Nat) (lr : Float := 0.1) : IO Unit := do
  ...
  IO.println s!"Initial weights W1: {state.w1Data.decode}"
  IO.println s!"Initial weights W2: {state.w2Data.decode}"
  IO.println ""
  ...
  IO.println ""
  IO.println s!"Final weights W1: {state.w1Data.decode}"
  IO.println s!"Final weights W2: {state.w2Data.decode}"
