...
private def assertRange (arr : FloatArray) (lo hi : Float64) (label : String) : IO Unit := do
  let mut ok := true
...
def runAll : IO Unit := do
  ...
  let denom := Float64.ofNat (max 1 (fanIn + fanOut))
  let bound := Float64.sqrt (6.0 / denom)
  ...
  let denomK := Float64.ofNat (max 1 fanInK)
  let gain := Float64.sqrt (2.0 / 1.0)
  let kbound := gain * Float64.sqrt (3.0 / denomK)
